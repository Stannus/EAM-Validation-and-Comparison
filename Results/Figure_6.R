...
eam <- read.csv("H:\\Toolkit\\Mirror\\Comparison\\EAM\\v3\\eam_bias.csv")

eam[,'method'] <- factor(eam[,'method'], levels = c("bias"))
eam[,'data'] <- factor(eam[,'data'], levels = c("ERA5", "JRA55", "NCEP2", "CRU"))
eam[,'significance'] <- factor(eam[,'significance'], levels = c(1, 0))

eam$coefficient <- (eam$coefficient)*(-1)
...
ggsave("G:/내 드라이브/ICP Lab/Comparison/Figures/figure_6a.png", device = 'png',
       plot = t, width = 14, height = 21, dpi = "retina", units = 'in')
ggsave("G:/내 드라이브/ICP Lab/Comparison/Figures/figure_6b.png", device = 'png',
